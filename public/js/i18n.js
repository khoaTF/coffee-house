...
function applyLanguage() {
    ...
    const map = {
        '#my-orders-btn span.font-bold': { text: t.my_orders },
        '#mobile-my-orders-btn span.font-bold': { text: t.my_orders },
        '#menu-search-desktop': { placeholder: t.search_placeholder },
        '#menu-search-mobile': { placeholder: t.search_placeholder },
        '#live-order-banner .text-sm.font-bold': { html: `<i class="fa-solid fa-clock pulse-icon text-primary me-2"></i> ${t.order_status}` },
        'button[onclick*="requestStaffService(\'staff\')"]': { html: `<i class="fa-solid fa-bell"></i> ${t.call_staff}` },
        'a[onclick*="requestStaffService(\'staff\')"] span:last-child': { text: t.call_staff },
        'button[onclick*="requestStaffService(\'water\')"]': { html: `<i class="fa-solid fa-glass-water"></i> ${t.water}` },
        'a[onclick*="requestStaffService(\'checkout\')"] span.font-bold': { text: t.checkout },
        '#view-cart-btn-docked span:first-child': { text: t.checkout },
        'a[data-category="All"] span': { text: t.filter_all },
        'button[data-category="All"]': { text: t.filter_all },
        '#loader': { html: `<i class="fa-solid fa-circle-notch fa-spin text-primary text-2xl mb-2"></i><br>${t.loading_menu}` },
