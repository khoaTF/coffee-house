...
export function renderMenu(category) {
    ...
    filteredItems.forEach((item, index) => {
        ...
        const fallbackLogo = window.tenantLogoUrl || '/images/bunny_logo.png';
        
        card.innerHTML = `
            <div class="img-wrap aspect-[4/3] bg-[#F0EDEC] dark:bg-slate-800 relative overflow-hidden">
                <img src="${item.imageUrl}" alt="${window.escapeHTML(item.name)}" loading="lazy" decoding="async" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onerror="this.onerror=null; this.outerHTML='<div class=\\'w-full h-full flex items-center justify-center p-6 bg-[#F0EDEC] dark:bg-slate-800\\'><img src=\\'${fallbackLogo}\\' alt=\\'\\'  class=\\'w-full h-full object-contain opacity-30 group-hover:scale-105 transition-transform duration-500\\'></div>';">
